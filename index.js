...
function createDictionary(entries) {
	...
	entries.forEach(([value, key]) => dictionary.set(value, key));
	return dictionary;
...
function checkPresence(dictionary, keys) {
	...
	keys.forEach((key) => {
		if (dictionary.has(key)) {
			arrPresence.push(true);
		} else {
			arrPresence.push(false);
		}
	});
	return arrPresence;
...
function sortByValues(dictionary) {
	const arrFromDict = Array.from(dictionary);
	arrFromDict.sort((a, b) => b[1] - a[1]);
	return new Map(arrFromDict);
...
function convertDictionaryToSet(dictionary) {
	const setFromDictKeys = new Set();
	for (let key of dictionary.keys()) {
		if (typeof key === "string") {
			setFromDictKeys.add(key.toUpperCase());
		} else if (typeof key === "number") {
			setFromDictKeys.add(key + 1);
		}
	}
	return setFromDictKeys;
...
function convertSetToDictionary(set) {
	const dictFromSet = new Map();
	for (let value of set) {
		dictFromSet.set(value, value.charCodeAt(0));
	}
	return dictFromSet;
